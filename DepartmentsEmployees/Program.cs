...
namespace DepartmentsEmployees
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            PrintEmployeeReport($"Employees in {firstDepartment.DeptName}", employees, firstDepartment);
            ...
            employees = repository.GetAllEmployeesWithDepartment();
            PrintEmployeeReport("All Employees after adding Jane", employees, firstDepartment);
            ...
            repository.UpdateEmployee(secondDepartment, dbJane);

            employees = repository.GetAllEmployeesWithDepartment();
            PrintEmployeeReport("All Employees after updating Jane", employees, secondDepartment);
        ...
        public static void PrintEmployeeReport(string title, List<Employee> employees)
        {
             ...
             if (title == "All Employees")
            {
                ...
                employees.ForEach(employee => Console.WriteLine($"{employee.FirstName} {employee.LastName}"));
            }
            else if (title == "All Employees with departments")
            {
                Console.WriteLine("All Employees with Departments");
                employees.ForEach(employee => Console.WriteLine($"{employee.FirstName} {employee.LastName}. Dept {employee.Department.DeptName}"));
            }
            else
            {
                Console.WriteLine($"{title}");
        ...
        public static void PrintEmployeeReport(string title,List<Employee> employees, Department dept)
        {
            Console.WriteLine($"{title}");

            List<Employee> employeesInCurrentDepartment =  employees.Where(emp => emp.DepartmentId == dept.Id).ToList();

            employeesInCurrentDepartment.ForEach(employee => Console.WriteLine($"{employee.FirstName} {employee.LastName}"));
        }
