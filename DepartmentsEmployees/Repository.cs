...
namespace DepartmentsEmployees.Data
{
    ...
    public class Repository
    {
        ...
        public List<Employee> GetAllEmployeesWithDepartment()
        {
            using (SqlConnection conn = Connection)
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    cmd.CommandText = "SELECT e.Id, e.FirstName, e.LastName, e.DepartmentId, d.DeptName FROM Employee e JOIN Department d ON e.DepartmentId = d.Id";
                    SqlDataReader reader = cmd.ExecuteReader();
        ...
        /// </summary>
        public void UpdateEmployee(Department dept, Employee emp)
        {
            /*
            ...
            using (SqlConnection conn = Connection)
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"UPDATE Employee
                                         ...
                                         WHERE Id = @id";
                    cmd.Parameters.Add(new SqlParameter("@deptId", dept.Id));
